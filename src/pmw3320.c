...
LOG_MODULE_REGISTER(pmw3320, CONFIG_INPUT_LOG_LEVEL);

//////// Sensor initialization steps definition //////////
// init is done in non-blocking manner (i.e., async), a //
// delayable work is defined for this purpose           //
enum pmw3320_init_step
{
...
static int pmw3320_async_init_power_up(const struct device *dev);
static int pmw3320_async_init_clear_ob1(const struct device *dev);
static int pmw3320_async_init_check_ob1(const struct device *dev);
static int pmw3320_async_init_configure(const struct device *dev);

static int (*const async_init_fn[ASYNC_INIT_STEP_COUNT])(const struct device *dev) = {
    [ASYNC_INIT_STEP_POWER_UP] = pmw3320_async_init_power_up,
    [ASYNC_INIT_STEP_CLEAR_OB1] = pmw3320_async_init_clear_ob1,
    [ASYNC_INIT_STEP_CHECK_OB1] = pmw3320_async_init_check_ob1,
    [ASYNC_INIT_STEP_CONFIGURE] = pmw3320_async_init_configure,
};
...
/* static int reg_write(const struct device *dev, uint8_t reg, uint8_t val) {
    ...
    // enable spi clock
    err = _reg_write(dev, PMW3320_REG_SPI_CLK_ON_REQ, PMW3320_SPI_CLOCK_CMD_ENABLE);
    if (unlikely(err != 0)) {
        return err;
    }
    ...
    err = _reg_write(dev, PMW3320_REG_SPI_CLK_ON_REQ, PMW3320_SPI_CLOCK_CMD_DISABLE);
    if (unlikely(err != 0)) {
        return err;
    }
...
static int motion_burst_read(const struct device *dev, uint8_t *buf, size_t burst_size)
{
    ...
    __ASSERT_NO_MSG(burst_size <= PMW3320_MAX_BURST_SIZE);
    ...
    uint8_t reg_buf[] = {PMW3320_REG_MOTION_BURST};
...
static int check_product_id(const struct device *dev)
{
    ...
    int err = reg_read(dev, PMW3320_REG_PRODUCT_ID, &product_id);
    if (err)
    {
    ...
    if (product_id != PMW3320_PRODUCT_ID)
    {
        LOG_ERR("Incorrect product id 0x%x (expecting 0x%x)!", product_id, PMW3320_PRODUCT_ID);
        return -EIO;
    }
...
static int set_cpi(const struct device *dev, uint32_t cpi)
{
    ...
    if ((cpi > PMW3320_MAX_CPI) || (cpi < PMW3320_MIN_CPI))
    {
    ...
    /* set the cpi */
    // uint8_t addr[] = {0x7F, PMW3320_REG_RES_STEP, 0x7F};
    // uint8_t data[] = {0xFF, value, 0x00};
    int err = reg_write(dev, PMW3320_REG_RESOLUTION, value);
    if (err)
    {
...
static int set_downshift_time(const struct device *dev, uint8_t reg_addr, uint32_t time)
{
    ...
    switch (reg_addr)
    {
    case PMW3320_REG_RUN_DOWNSHIFT:
        /*
         * Run downshift time = PMW3320_REG_RUN_DOWNSHIFT
...
static int pmw3320_async_init_power_up(const struct device *dev)
{
    ...
    return reg_write(dev, PMW3320_REG_POWER_UP_RESET, PMW3320_POWERUP_CMD_RESET);
}
...
static int pmw3320_async_init_clear_ob1(const struct device *dev)
{
    ...
    return reg_write(dev, PMW3320_REG_OBSERVATION, 0x00);
}
...
static int pmw3320_async_init_check_ob1(const struct device *dev)
{
    ...
    /*     uint8_t value;
        int err = reg_read(dev, PMW3320_REG_OBSERVATION, &value);
        if (err) {
...
static int pmw3320_async_init_configure(const struct device *dev)
{
    ...
    if (!err)
    {
        err = set_cpi(dev, CONFIG_PMW3320_CPI);
    }

    // set performace register: run mode, vel_rate, poshi_rate, poslo_rate
    if (!err)
    {
        err = reg_write(dev, PMW3320_REG_PERFORMANCE, PMW3320_PERFORMANCE_VALUE);
        LOG_INF("Set performance register (reg value 0x%x)", PMW3320_PERFORMANCE_VALUE);
    }

    // required downshift and rate registers
    if (!err)
    {
        err = set_downshift_time(dev, PMW3320_REG_RUN_DOWNSHIFT,
                                 CONFIG_PMW3320_RUN_DOWNSHIFT_TIME_MS);
    }
    if (!err)
    {
        err = set_sample_time(dev, PMW3320_REG_REST1_PERIOD, CONFIG_PMW3320_REST1_SAMPLE_TIME_MS);
    }
    if (!err)
    {
        err = set_downshift_time(dev, PMW3320_REG_REST1_DOWNSHIFT,
                                 CONFIG_PMW3320_REST1_DOWNSHIFT_TIME_MS);
    }

    // downshift time for each rest mode
#if CONFIG_PMW3320_REST2_DOWNSHIFT_TIME_MS > 0
    if (!err)
    {
        err = set_downshift_time(dev, PMW3320_REG_REST2_DOWNSHIFT,
                                 CONFIG_PMW3320_REST2_DOWNSHIFT_TIME_MS);
    }
#endif
#if CONFIG_PMW3320_REST2_SAMPLE_TIME_MS >= 10
    if (!err)
    {
        err = set_sample_time(dev, PMW3320_REG_REST2_PERIOD, CONFIG_PMW3320_REST2_SAMPLE_TIME_MS);
    }
#endif
#if CONFIG_PMW3320_REST3_SAMPLE_TIME_MS >= 10
    if (!err)
    {
        err = set_sample_time(dev, PMW3320_REG_REST3_PERIOD, CONFIG_PMW3320_REST3_SAMPLE_TIME_MS);
    }
#endif
    if (err)
    {
...
// checked and keep
static void pmw3320_async_init(struct k_work *work)
{
    ...
    LOG_INF("PMW3320 async init step %d", data->async_init_step);

    data->err = async_init_fn[data->async_init_step](dev);
    if (data->err)
    {
        LOG_ERR("PMW3320 initialization failed");
    }
    else
    {
...
static int pmw3320_report_data(const struct device *dev)
{
    struct pixart_data *data = dev->data;
    uint8_t buf[PMW3320_BURST_SIZE];
    ...
    {
    case MOVE:
        set_cpi_if_needed(dev, CONFIG_PMW3320_CPI);
        dividor = CONFIG_PMW3320_CPI_DIVIDOR;
        break;
    case SCROLL:
        set_cpi_if_needed(dev, CONFIG_PMW3320_CPI);
        ...
        break;
    case SNIPE:
        set_cpi_if_needed(dev, CONFIG_PMW3320_SNIPE_CPI);
        dividor = CONFIG_PMW3320_SNIPE_CPI_DIVIDOR;
        break;
    default:
        return -ENOTSUP;
    }
    ...
    int16_t raw_x =
        TOINT16((buf[PMW3320_X_L_POS] + ((buf[PMW3320_XY_H_POS] & 0xF0) << 4)), 12) / dividor;
    int16_t raw_y =
        TOINT16((buf[PMW3320_Y_L_POS] + ((buf[PMW3320_XY_H_POS] & 0x0F) << 8)), 12) / dividor;

    int16_t x;
    int16_t y;

    if (IS_ENABLED(CONFIG_PMW3320_ORIENTATION_0))
    {
        x = -raw_x;
        y = raw_y;
    }
    else if (IS_ENABLED(CONFIG_PMW3320_ORIENTATION_90))
    {
        x = raw_y;
        y = -raw_x;
    }
    else if (IS_ENABLED(CONFIG_PMW3320_ORIENTATION_180))
    {
        x = raw_x;
        y = -raw_y;
    }
    else if (IS_ENABLED(CONFIG_PMW3320_ORIENTATION_270))
    {
        x = -raw_y;
        y = raw_x;
    }
    ...
    if (x != 0 || y != 0)
    {
        ...
        {
            data->scroll_delta += y;
            if (abs(data->scroll_delta) > CONFIG_PMW3320_SCROLL_TICK)
            {
...
static void pmw3320_gpio_callback(const struct device *gpiob, struct gpio_callback *cb,
                                  uint32_t pins)
{
...
static void pmw3320_work_callback(struct k_work *work)
{
    struct pixart_data *data = CONTAINER_OF(work, struct pixart_data, trigger_work);
    const struct device *dev = data->dev;

    pmw3320_report_data(dev);
    set_interrupt(dev, true);
}

static int pmw3320_init_irq(const struct device *dev)
{
    ...
    gpio_init_callback(&data->irq_gpio_cb, pmw3320_gpio_callback, BIT(config->irq_gpio.pin));
...
static int pmw3320_init(const struct device *dev)
{
    ...
    k_work_init(&data->trigger_work, pmw3320_work_callback);
    ...
    k_work_init_delayable(&data->init_work, pmw3320_async_init);
...
#define PMW3320_DEFINE(n)                                                                       \
    ...
    static const struct pixart_config config##n = {                                             \
        ...
        .snipe_layers_len = DT_PROP_LEN(DT_DRV_INST(n), snipe_layers),                          \
                                                                                                ...
                                                                                                \
    DEVICE_DT_INST_DEFINE(n, pmw3320_init, NULL, &data##n, &config##n, POST_KERNEL,             \
                          CONFIG_SENSOR_INIT_PRIORITY, NULL);

DT_INST_FOREACH_STATUS_OKAY(PMW3320_DEFINE)
